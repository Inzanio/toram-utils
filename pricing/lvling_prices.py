...
with tab_by_level :
    ...
    currentRaise = .1
    def pricing(level):
        price_from_exp = exp_require_for_next_lvl(level)/(1000*(1-currentRaise))
        price_from_exp = price_from_exp * (25*(1+currentRaise))
        return price_ceil((level * (2000*(1+currentRaise)) + price_from_exp )//discount["by_level"])
